...
data.dropna(subset = ["pass_good"], inplace = True, axis = 0)
print(data.shape)

#pos_cat = pd.get_dummies(data["pos"])
independent = pd.concat([data.loc[:,"Acceleration_y":"Volleys"], data.loc[:,"Acceleration_y_own":"Volleys_other"]], axis = 1)

dependent_list = ["pass_good","pass_long","pass_short","clearance"]
...
regression.loc[regression["variable"] == "Intercept", "se"] = np.nan 
regression.loc[regression["variable"] == "Intercept", "pval"] = np.nan
...
regression.loc[(regression["pval"] < 0.05) | (regression["variable"] == "Intercept"), "keep"] = 1
...
regression.to_csv("output/regression_GK.csv", index = False)