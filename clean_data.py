...
for o in objects:
    ...
    data[o] = pd.to_numeric(data[o].str[:2])
    o_2 = data[o].isna().sum()
    print("Loss is: {} for {}".format(o_2-o_1,o))

print(objects)
for o in objects:
    data_o = data.groupby(["wy_id_x","home"])[o].mean().reset_index()
    data_o = data_o.pivot(index = "wy_id_x", columns = "home", values = o).reset_index()
    colname_home = o + "_away"
    colname_away = o + "_home"
    #colname_away = "{o}_away".format(o)
    data_o.columns = ["wy_id_x", colname_home, colname_away]

    #    print(type(data_agression))
    #    print(data_agression)
    data = data.merge(data_o, how = "left", on = "wy_id_x")
...
print(data.shape)
data.drop_duplicates(subset = ["wy_id_x","player_id"], inplace = True)
print(data.shape)

data_analysis = pd.concat([data.loc[:,["wy_id_x","player_id"]],data.loc[:,"Acceleration_y":"Volleys"],data.loc[:,"Acceleration_y_own":"Volleys_other"]], axis = 1)
...
data = data.merge(data_nonmissing[["wy_id_x","player_id","filter"]], how = "right", on = ["wy_id_x", "player_id"])
print(data.shape)
